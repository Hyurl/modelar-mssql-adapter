...
describe("Model.prototype.has() & Model.prototype.belongsTo()", function () {
    it("should create model associations as expected", function (done) {
        ...
        co(function* () {
            ...
            assert.equal(article.sql, 'select top 1 * from [articles2] where [user_id] = ?');
            ...
            assert.equal(_user.sql, 'select top 1 * from [users2] where [id] = ?');
            ...
            assert.equal(userTag.sql, 'select top 1 * from [tags2] where [taggable_id] = ? and [type] = ?');
            ...
            assert.equal(user2.sql, 'select top 1 * from [users2] where [id] = ?');
